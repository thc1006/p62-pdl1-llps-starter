#!/usr/bin/env python3
"""
Stage 2: Real Multivariate Cox Survival Analysis (Ensembl ID compatible)
...
if sample_col.startswith('ENSG'):
    print("  ⚠️  Detected Ensembl IDs - will convert to gene symbols")

    # Rename Ensembl columns to gene symbols
    rename_dict = {}
    ...
    if rename_dict:
        ...
        print("  ✗ ERROR: Could not find target genes in expression matrix")
        print(f"     Available columns: {expr_df.columns[:10].tolist()}")
        raise ValueError("Target genes not found in expression matrix")
...
def load_tcga_clinical():
    ...
    """
    # Try from symbolic link first
    clinical_file = Path("outputs/tcga_full_cohort/clinical_data.csv")

    if clinical_file.exists():
        print(f"  Loading from: {clinical_file}")
        clinical = pd.read_csv(clinical_file)

        # Check if it has survival data
        if 'vital_status' in clinical.columns or 'days_to_death' in clinical.columns:
            return clinical
        else:
            print("  ⚠️  Clinical file exists but lacks survival data")

    # Try real cohort clinical data
    clinical_file2 = Path("outputs/tcga_full_cohort_real/clinical_data_full_real.csv")
    if clinical_file2.exists():
        print(f"  Loading from: {clinical_file2}")
        clinical = pd.read_csv(clinical_file2)

        if 'vital_status' in clinical.columns or 'days_to_death' in clinical.columns:
            return clinical

    # Fallback to simulation
    print("  Clinical data with survival info not found.")
    print("  Using FALLBACK: Will generate realistic survival simulation")
...
def generate_realistic_survival_simulation(expr_df):
    ...
    # Extract sample metadata from sample_id
    sample_ids = expr_df['sample_id'].values

    # Determine cancer type from sample ID
    cancer_types = []
    for sid in sample_ids:
        if 'LUAD' in sid:
            ...
            # Random assignment
            cancer_types.append(np.random.choice(['LUAD', 'LUSC', 'SKCM']))

    # Simulate realistic clinical variables
    clinical = pd.DataFrame({
        'submitter_id': sample_ids,
